...
public class LocationUtil {
    ...
    public static LinkedHashMap<String, Map<String, String>> getDistance(String key, List<String> origins, String destination, String type) throws Exception {
        ...
        if (origins != null && origins.size() != 0 && !StringUtils.isNotBlank(destination) && !StringUtils.isNotBlank(type)) {
            ...
            maps.stream().sorted((m1, m2) -> {
                if (new Integer(m1.get("distance")) > new Integer(m2.get("distance"))) {
                    return 1;
                }
                if (new Integer(m1.get("distance")).equals(new Integer(m2.get("distance")))) {
                    return 0;
                }
                return -1;
            }).forEach(map -> {
